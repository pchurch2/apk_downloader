...
def jadx():
    ...
    start_dir = apk_dir                    # Used for normal JADX use
    #start_dir = "apks_20200112-205248"      # Used only for one time JADX use
    jadx_timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    # Preston
    jadx_bin_dir = "/store/church/git/apk_downloader/jadx/build/jadx/bin/jadx"  # Include only beginning /
    jadx_apk_downloader_dir = "/store/church/git/apk_downloader/"               # Include beginning and ending /
    ...
    for dir_name, sub_dir_list, file_list in os.walk(start_dir):
        ...
        for file_name in file_list:
            ...
            # Refactored to create decompiled dir
            subprocess.call([jadx_bin_dir, "-d", "decompiled_" + jadx_timestamp + "/" + dir_name + "/" + file_name, jadx_apk_downloader_dir + dir_name + "/" + file_name])
